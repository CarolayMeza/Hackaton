...
public class inversionistaDAO {
	...
	public boolean Crear_Cliente(inversionistaDTO cliente) {
		boolean resul = false;
		try {
	...
	public inversionistaDTO Buscar_Cliente(String cedula) {
...
//			JOptionPane.showMessageDialog(null,"Error al consultar" +e.getMessage());
	...
	public boolean Actualizar_Cliente(inversionistaDTO user) {
		boolean resul = false;
		try {
	...
	public ArrayList<inversionistaDTO> cargar_select(){
		...
		inversionistaDTO cliente=null;
			...
			res=ps.executeQuery();
			while(res.next()) {
				cliente= new inversionistaDTO(res.getString(1),res.getString(2),res.getString(3),res.getString(4),res.getString(5));
			    lista2.add(cliente);
			}
			}catch(SQLException ex) {}
